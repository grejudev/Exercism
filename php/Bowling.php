...
class Game
{
    ...
    public function score(): int
    {
        ...
        $frame = 1;
        $current = 0;
        while ($current < count($this->pins) && $frame <= LAST_FRAME) {
            [$score, $current] = $this->frameScore($frame++, $current);
            $scores += $score;
        }
        if ($frame <= LAST_FRAME) {
            throw new Exception();
    ...
    private function frameScore(int $frame, int $current): array
    {
        if (!$this->isValidFrame($frame, $current)) {
            throw new Exception();
        };
        return $this->isStrike($current) ?
            [$this->sum($current, 3), $current + 1] :
            [$this->sum($current, $this->isSpare($current) ? 3 : 2), $current + 2];
    }
